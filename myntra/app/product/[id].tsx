...
export default function ProductDetails() {
  ...
  const autoScrollTimer = useRef<NodeJS.Timeout | number | null>(null);
  const { user } = useAuth();
    ...
    const fetchproduct = async () => {
      try {
        ...
        const product = await axios.get(
          API_ENDPOINTS.GET_PRODUCT(id as string),
        );
  ...
  const handleAddwishlist = async () => {
    ...
    try {
      await axios.post(API_ENDPOINTS.ADD_TO_WISHLIST, {
        userId: user._id,
        productId: id,
      });
      setiswishlist(true);
  ...
  const handleAddToBag = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(API_ENDPOINTS.ADD_TO_BAG, {
        userId: user._id,
        productId: id,
        size: selectedSize,
        quantity: 1,
